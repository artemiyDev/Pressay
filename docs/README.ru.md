...
- ограниченная проверка микрофона, которая различает тишину, слишком тихий
  сигнал, нормальный уровень и перегрузку без распознавания и сохранения аудио;
...
На Windows пункт **Системный микрофон по умолчанию** предпочитает системный
WASAPI-вход и один раз безопасно возвращается к штатному default PortAudio,
если WASAPI-вход не открылся. В списке микрофонов представления одного
физического устройства через MME, DirectSound, WASAPI и WDM-KS объединяются,
а WASAPI помечается как рекомендуемый. Явно выбранный микрофон остаётся
fail-closed: Pressay не подменяет его другим устройством незаметно.
